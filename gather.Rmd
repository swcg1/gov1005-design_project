...
census_2019_model <- left_join(census_2019_raw, salary_join, by = "salary") %>%
  select(age, gender, career_duration, org_size, department_size, design_satisfaction_level, salary)
...
model_salary <- lm(salary ~ age + gender + career_duration + org_size + department_size + highest_education + job_satisfaction_level, data = census_2017)

model_salary %>%
  tidy()

predict(model_salary, tibble(age = 22, 
                             gender = "Female", 
                             career_duration = "1-4 years", 
                             org_size = "11-50 employees", 
                             department_size = "Just me", 
                             highest_education = "Bachelor's degree",
                             job_satisfaction_level = "Pretty good"),
        interval = "confidence")
...
satisfaction_levels_2017 <- tibble(
  ...
  level_num = c(2, 3, 4, 1, NA)
)

model_satisfaction_data_2017 <- left_join(census_2017, satisfaction_levels_2017, by = "job_satisfaction_level") %>%
  filter(job_satisfaction_level != "Not sure")

satisfaction_levels_2019 <- tibble(
  design_satisfaction_level = c("Not really satisfied, but I deal" , "Happy, but there's room for improvement", "Extremely satisfied with my work situation", "Ready to call it quits", "Not sure"),
  level_num = c(2, 3, 4, 1, NA)
)

model_satisfaction_data_2019 <- left_join(census_2019, satisfaction_levels_2019, by = "design_satisfaction_level") %>%
  filter(job_satisfaction_level != "Not sure")

model_satisfaction <- lm(level_num ~ age + gender + career_duration + org_size + department_size + highest_education + salary, data = model_satisfaction_data_2017)
